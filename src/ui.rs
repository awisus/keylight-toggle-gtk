...
impl KeylightUI {
    pub fn new(repository: &Arc<dyn Repository>) -> Self {
        ...
        let exit_button = MenuItem::with_label("Exit");
        menu.append(&exit_button);
        let mut indicator = AppIndicator::new("tray-sliders", &indictor_icon());
        indicator.set_menu(&mut menu);

        let ui = Self {
            repository: repository.clone(),
            on: Rc::new(RefCell::new(None)),
            lights: Rc::new(RefCell::new(vec![])),
    ...
    fn discover(&self) {
        ...
        glib::MainContext::default().spawn_local(async move {
            toggle_button.borrow_mut().set_label("Suche...");
            toggle_button.borrow_mut().set_sensitive(false);
            ...
            if let Ok(found_lights) = repo.list().await {
                if let Some(first) = found_lights.first() {
                    ...
                    *lights.borrow_mut() = found_lights;
                    toggle_button.borrow_mut().set_label(&toggle_label(is_on));
                    toggle_button.borrow_mut().set_sensitive(true);
                } else {
                    toggle_button.borrow_mut().set_label("Keine Keylights");
                }
            } else {
                toggle_button.borrow_mut().set_label("Keine Verbindung");
            }
        });
    }
}
